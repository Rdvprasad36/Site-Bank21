...
export function ContactBar({
  ...
  const shouldShow = Boolean(agentPhone || agentWhatsapp);
      ...
      <div className={`md:hidden fixed bottom-0 inset-x-0 bg-white border-t shadow-[0_-8px_24px_rgba(0,0,0,0.06)] p-4 pb-safe flex gap-3 z-50 animate-slide-in-up ${isTransitioning ? 'transition-none' : 'transition-all duration-300'} ${isKeyboardOpen ? 'translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
        ...
        {wa && (
          <a
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20ba59] active:scale-95 text-white font-semibold py-3.5 rounded-xl text-sm transition-all shadow-md shadow-emerald-500/10"
          >
        ...
        {tel && (
          <a
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-semibold py-3.5 rounded-xl text-sm transition-all shadow-md shadow-blue-500/10"
          >
      ...
      <div className="hidden md:flex gap-2.5">
        {wa && (
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => trackClick(slug, 'CLICK_WHATSAPP')}
            className="inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#20ba59] active:scale-95 text-white font-semibold px-4.5 py-2.5 rounded-xl text-sm transition-all shadow-sm"
          >
        ...
        {tel && (
          <a
            href={tel}
            onClick={() => trackClick(slug, 'CLICK_CALL')}
            className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-semibold px-4.5 py-2.5 rounded-xl text-sm transition-all shadow-sm"
          >
